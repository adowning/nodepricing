...
exports.getbasicsettings = function(req, res) {
    var user = req.user;
    var opr;
    var thiskey;
    var monthlytotals = new Object();
    ...
    Company.findOne({
        ...
        OnlinePrice.find({
            ...
            var o = opr;
            res.render('onlinepricing/edit_basicsettings', {
                services: o
            });
...
exports.getpricetables = function(req, res) {
    var user = req.user;
    var opr;
    var thiskey;
    var monthlytotals = new Object();
    ...
    Company.findOne({
        ...
        OnlinePrice.find({
            ...
            var o = opr;
            res.render('onlinepricing/edit_pricetables', {
                services: o
            });
...
exports.getvisuallook = function(req, res) {
    var user = req.user;
    var opr;
    var thiskey;
    var monthlytotals = new Object();
    ...
    Company.findOne({
        ...
        OnlinePrice.find({
            ...
            var o = opr;
            res.render('onlinepricing/edit_visuallook', {
                services: o
            });
...
exports.getreports = function(req, res) {
    ...
    Company.find(
        function(err, companies) {
            ...
            var comps = companies;
            res.render('onlinepricing/getreports', {
                services: comps
            });
        ...
        Company.findOne({
            ...
            OnlinePrice.findOne({
                ...
                if (bs[0] == 'bs') {
                    // console.log('bs = bs');
                    res.contentType('json');
                    res.send({ // console.log(company.servicemonster);
                        // if(company.servicemonster){
                        //     console.log('this comp uses sm');
                        // }else{
                        //     console.log('this company does not use sm');
                        // }
                        data: company
                        //comp: comp
                    });
                } else {
                    // console.log(company.servicemonster);
                    // if(company.servicemonster){
                    //     console.log('this comp uses sm');
                    // }else{
                    //     console.log('this company does not use sm');
    ...
    if (req.path == '/onlinepricing') {
        ...
        async.series({
                one: function(callback) {
                    ...
                    // }, function(err, company) {
                    //     if (!company) {
                    //         console.log('oops no company');
                    //     }
                    //     usesm = company.servicemonster;
                    //     console.log(usesm);
                    //     console.log('2');
                    // });

                    //
                    //FIX THE FUCK OUT OF MEE
                    //
                    callback(null, 1);
                ...
                two: function(callback) {
                    ...
                    OnlinePrice.findOne({
                        ...
                        if (!top) {
                            console.log('no op here!!');
                            req.flash('error', "Cannot find your pricing model");
                            return res.redirect('/badkey');
                        }
                        console.log('got op');
                        op = top;
                        console.log('hiyas' + usesm);
                        //TODO fix me
                        usesm = "penis";
                        if (usesm || usesm == "penis") {
                            console.log('this comp uses sm');

                            op.bookedslots = getSMBookedSlots();
...
function saveCompany(company) {
    company.save(function(err, company) {
        if (err && err.code == 11001) {
            ...
            req.flash('error', "That " + duplicatedAttribute + " is already in use.");
            console.log('error');
            return res.redirect('/getSMBookedSlotsupdateavailability');
        }
        if (err) return next(err);
        //req.flash('success', "Availibity updated successfully.");
